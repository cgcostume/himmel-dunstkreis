 ...
 * scattering model. Ported from osgHimmel's `t_modelCfg`
 * (atmosphereprecompute.h), same units and defaults (Earth-like
 * atmosphere, wavelengths implicit in the RGB scattering coefficients).
 */
...
/**
 * Resolution/sampling configuration for the precomputed LUTs. Ported from
 * osgHimmel's `t_preTexCfg` (atmosphereprecompute.h). The original
 * generated these via multi-pass FBO renders; here they become compute
 * shader dispatch/storage-texture sizes.
 */
